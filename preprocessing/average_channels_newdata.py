...
# Step 1: Load MEG channels present in all trials
channels_intersection = []
with open("channels.txt") as channel_file:
    for line in channel_file:
        channels_intersection.append(line.strip())
...
for subject_dir in sorted(glob.glob(os.path.join('new_data', '*'))):
...
for subject_dir in tqdm(sorted(glob.glob(os.path.join('new_data', '*')))):
    for trial_dir in sorted(glob.glob(os.path.join(subject_dir, 'MNI*'))):
        ...
        for epoch_address in sorted(glob.glob(os.path.join(trial_dir, 'data_block*'))):
            ...
            for area in areas.keys():
                ...
                mean_channel = sum_channels / len(areas[area])
                epoch_df[area] = pd.Series(mean_channel)
            
            epoch_df = epoch_df.transpose()
            relative_path = os.path.relpath(epoch_address, start='new_data')
